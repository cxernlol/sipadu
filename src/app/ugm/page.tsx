...
export default function UGMPage() {
  return (
    <div className="min-h-screen bg-[#f8f8fa] dark:bg-black relative overflow-hidden">
      {/* Ambient background */}
      <div className="fixed inset-0 -z-10">
        <div className="absolute top-1/4 left-1/3 w-[500px] h-[500px] bg-blue-200/25 dark:bg-blue-900/20 rounded-full blur-[128px] animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-cyan-200/20 dark:bg-cyan-900/10 rounded-full blur-[128px] animate-pulse [animation-delay:2s]" />
      ...
      <main className="flex-1 flex items-center justify-center min-h-[calc(100vh-73px)]">
        <AnimateIn className="text-center max-w-2xl px-6 flex flex-col items-center">
          {/* University badge */}
          <div className="inline-flex items-center gap-2.5 bg-white dark:bg-zinc-900 px-5 py-2.5 rounded-full text-sm font-medium text-blue-800 dark:text-blue-400 border border-blue-200 dark:border-blue-800 shadow-sm mb-10">
            <img src="https://upload.wikimedia.org/wikipedia/commons/b/b3/Lambang_UGM.svg" alt="UGM" className="h-5 w-5 object-contain" />
            Universitas Gadjah Mada
          </div>

          {/* Construction icon */}
          <div className="h-24 w-24 rounded-3xl bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 flex items-center justify-center mb-10">
            <i className="fa-solid fa-helmet-safety text-4xl text-blue-400" />
          </div>

          <h1 className="text-4xl md:text-5xl font-bold tracking-tight leading-tight mb-6 text-slate-900 dark:text-zinc-50">
            Portal sedang dalam<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500">tahap pengembangan.</span>
          </h1>

          <p className="text-lg text-slate-400 dark:text-zinc-400 mb-10 leading-relaxed font-light max-w-lg">
            Tim engineering kami sedang memetakan dan mengintegrasikan sistem informasi Universitas Gadjah Mada ke dalam jaringan SiPadu.
          </p>
