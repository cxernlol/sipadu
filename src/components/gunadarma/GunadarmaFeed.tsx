    ...
    <div className="w-full">
      ...
      <div className="mb-8">
        <h3 className="font-semibold text-slate-900 mb-4 flex items-center gap-2">
          <i className="fa-solid fa-filter text-slate-400"></i> Filter Sumber
        </h3>
        <div className="flex flex-wrap gap-2">
          ...
          {selectedTags.length > 0 && (
            <Badge
              variant="ghost"
              className="cursor-pointer px-4 py-1.5 rounded-full text-xs font-medium text-slate-400 hover:text-slate-600 hover:bg-slate-100"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAnnouncements.length > 0 ? (
          filteredAnnouncements.map((announcement, idx) => (
            <AnimateIn key={announcement.id} delay={idx * 0.05}>
              <div
                ...
                className="bg-white border border-slate-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer group flex flex-col h-full"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="bg-slate-50 border border-slate-100 text-slate-500 text-xs font-bold px-3 py-2 rounded-lg text-center leading-none">
                    <span className="block text-lg text-slate-800 mb-0.5">{announcement.date.day}</span>
                    {announcement.date.month}
                  </div>
                  <div className="flex flex-wrap gap-1 justify-end">
                    {announcement.tags?.map((tag) => (
                      <span key={tag} className="text-[9px] uppercase font-bold tracking-wider bg-blue-50 text-blue-600 px-2 py-1 rounded">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <h3 className="font-semibold text-slate-900 mb-3 line-clamp-3 group-hover:text-blue-600 transition-colors">
                  {announcement.title}
                </h3>
                ...
                <div className="mt-6 text-blue-600 text-sm font-medium flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="col-span-full py-12 text-center text-slate-500 bg-slate-50 rounded-xl border border-dashed border-slate-200">
            <i className="fa-solid fa-folder-open text-4xl text-slate-300 mb-4 block mx-auto"></i>
          ...
          <DialogHeader className="mb-4">
            <div className="flex gap-2 mb-4">
              {selectedAnnouncement?.tags?.map((tag) => (
                <span key={tag} className="text-[10px] uppercase font-bold tracking-wider bg-blue-50 text-blue-600 px-2 py-1 rounded">
                  {tag}
                </span>
              ))}
            </div>
            <DialogTitle className="text-2xl leading-tight font-serif text-slate-900">
